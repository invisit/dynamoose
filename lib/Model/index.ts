...
import {Schema, SchemaSettings, SchemaDefinition, DynamoDBSetTypeResult, ValueType, IndexItem} from "../Schema";
import {
	Document as DocumentCarrier,
...
export enum ModelUpdateOptions {
  ttl = "ttl",
  indexes = "indexes",
...
export async function getTableDetails (
	model:Model<DocumentCarrier>,
	settings:{ allowError?:boolean; forceRefresh?:boolean } = {}
...
export async function createTable (model:Model<DocumentCarrier>):Promise<void | (() => Promise<void>)> {
	if ((
		(
			await getTableDetails(model, {"allowError": true}) || {}
		).Table || {}
	).TableStatus === "ACTIVE") {
	...
	function updateTTL ():Promise<DynamoDB.UpdateTimeToLiveOutput> {
		return ddb("updateTimeToLive", {
			...
			"TimeToLiveSpecification": {
				"AttributeName": (
          model.options.expires as ModelExpiresSettings
				).attribute,
				"Enabled": true
			}
		});
			...
			try {
				...
				const tableDetails = (
					await getTableDetails(
						model,
						{"forceRefresh": forceRefreshOnFirstAttempt === true ? forceRefreshOnFirstAttempt : count > 0}
					)
				).Table;
				if (tableDetails.TableStatus ===
          "ACTIVE" &&
          (
          	tableDetails.GlobalSecondaryIndexes ?? []
          ).every((val) => val.IndexStatus === "ACTIVE")) {
					return resolve();
				}
			} catch (e) {
				return reject(e);
			...
			if (count > 0) {
				model.options.waitForActive.check.frequency === 0 ?
			...
			if (Date.now() - start >= model.options.waitForActive.check.timeout) {
				return reject(new CustomError.WaitForActiveTimeout(`Wait for active timed out after ${Date.now() -
        start} milliseconds.`));
			} else {
				check(++count);
			}
...
export async function updateTable (model:Model<DocumentCarrier>):Promise<void> {
	...
	if (updateAll ||
    (
      model.options.update as ModelUpdateOptions[]
    ).includes(ModelUpdateOptions.throughput)) {
		const currentThroughput = (
			await getTableDetails(model)
		).Table;
		const expectedThroughput:any = utils.dynamoose.get_provisioned_throughput(model.options);
		const isThroughputUpToDate = expectedThroughput.BillingMode ===
      (
      	currentThroughput.BillingModeSummary || {}
      ).BillingMode &&
      expectedThroughput.BillingMode ||
      (
      	currentThroughput.ProvisionedThroughput || {}
      ).ReadCapacityUnits ===
      (
      	expectedThroughput.ProvisionedThroughput || {}
      ).ReadCapacityUnits &&
      currentThroughput.ProvisionedThroughput.WriteCapacityUnits ===
      expectedThroughput.ProvisionedThroughput.WriteCapacityUnits;
    ...
    (
      model.options.update as ModelUpdateOptions[]
    ).includes(ModelUpdateOptions.indexes)) {
...
export class Model<T extends DocumentCarrier = AnyDocument> {
    ...
    public readonly schemaSettings: Partial<SchemaSettings> = {}
	) {
		...
		if (!schema || Array.isArray(schema) && schema.length === 0) {
          ...
          schema.map((schema:Schema | SchemaDefinition):Schema => schema instanceof Schema ?
          	schema :
          	new Schema(schema, schemaSettings));
			} else {
				realSchemas = [new Schema(schema, schemaSettings)];
			}
		} else {
			realSchemas = [schema];
		}
		...
		if (options.expires) {
			...
			realSchemas.forEach((schema) => {
				schema.schemaObject[(
          options.expires as ModelExpiresSettings
				).attribute] = {
					"type": {
						"value": Date,
						"settings": {
		...
		if ((
			this.options.create ||
      (
      	Array.isArray(this.options.update) ? this.options.update.includes(ModelUpdateOptions.ttl) : this.options.update
      )
		) && options.expires) {
      ...
      this.Document as any
			...
			{"key": "delete"},
			{
				"key": "update",
				"settingsIndex": 2,
				"modifier": (response:DynamoDB.UpdateItemInput):DynamoDB.UpdateItemInput => {
			...
			},
			{
				"key": "condition",
				"settingsIndex": -1,
				"dynamoKey": "ConditionCheck",
				"function": (key:string, condition:Condition):DynamoDB.ConditionCheck => (
          {
			...
			accumulator[key] = async (...args):Promise<DynamoDB.TransactWriteItem> => {
				if (typeof args[args.length - 1] === "function") {
					console.warn(
						"Dynamoose Warning: Passing callback function into transaction method not allowed. Removing callback function from list of arguments.");
					args.pop();
				}
  ...
  methods:{
    document:{ set:(name:string, fn:FunctionType) => void; delete:(name:string) => void }; set:(
      name:string,
      fn:FunctionType
    ) => void; delete:(name:string) => void
  };
  ...
  async schemaForObject (object:ObjectType):Promise<Schema> {
  	const schemaCorrectnessScores:number[] = this.schemas.map((schema) => schema.getTypePaths(
  		object,
  		{"type": "toDynamo", "includeAllProperties": true}
  	)).map((obj) => Object.values(obj).map((obj) => obj?.matchCorrectness || 0)).map((array) => Math.min(...array));
  ...
  async getIndexes ():Promise<{ GlobalSecondaryIndexes?:IndexItem[]; LocalSecondaryIndexes?:IndexItem[] }> {
  	return (
  		await Promise.all(this.schemas.map((schema) => schema.getIndexes(this)))
  ...
  batchGet (
    keys:InputKey[],
    settings:ModelBatchGetSettings & { "return":"request" },
  ...
  batchGet (
    ...
    callback:CallbackType<ModelBatchGetDocumentsResponse<T>, AWSError>
  ):void;
  batchGet (
    keys:InputKey[],
    settings:ModelBatchGetSettings & { "return":"documents" }
  ...
  batchGet (
  	keys:InputKey[],
  	settings?:ModelBatchGetSettings | CallbackType<ModelBatchGetDocumentsResponse<T>, AWSError> | CallbackType<DynamoDB.BatchGetItemInput, AWSError>,
  	callback?:CallbackType<ModelBatchGetDocumentsResponse<T>, AWSError> | CallbackType<DynamoDB.BatchGetItemInput, AWSError>
  ):void | DynamoDB.BatchGetItemInput | Promise<ModelBatchGetDocumentsResponse<T>> {
  	...
  	const prepareResponse = async (response:DynamoDB.BatchGetItemOutput):Promise<ModelBatchGetDocumentsResponse<DocumentCarrier>> => {
  		...
  		return keyObjects.reduce((result, key) => {
  			...
  			const item = tmpResult.find((item) => keyProperties.every((keyProperty) => item[keyProperty] ===
          key[keyProperty]));
  			if (item) {
  				result.push(item);
  			} else {
  				const item = tmpResultUnprocessed.find((item) => keyProperties.every((keyProperty) => item[keyProperty] ===
            key[keyProperty]));
  				if (item) {
  	...
  	if (callback) {
  		...
  		promise.then((response) => prepareResponse(response))
  			.then((response) => localCallback(null, response))
  			.catch((error) => localCallback(error));
  	} else {
  		return (
  			async ():Promise<ModelBatchGetDocumentsResponse<T>> => {
  ...
  batchPut (
    ...
    settings:ModelBatchPutSettings & { "return":"request" }
  ...
  batchPut (
    ...
    callback:CallbackType<{ "unprocessedItems":ObjectType[] }, AWSError>
  ):void;
  batchPut (
    documents:ObjectType[],
    settings:ModelBatchPutSettings & { "return":"response" }
  ):Promise<{ "unprocessedItems":ObjectType[] }>;
  batchPut (
    documents:ObjectType[],
    settings:ModelBatchPutSettings & { "return":"response" },
    callback:CallbackType<{ "unprocessedItems":ObjectType[] }, AWSError>
  ...
  batchPut (
  	documents:ObjectType[],
  	settings?:ModelBatchPutSettings | CallbackType<{ "unprocessedItems":ObjectType[] }, AWSError> | CallbackType<DynamoDB.BatchWriteItemInput, AWSError>,
  	callback?:CallbackType<{ "unprocessedItems":ObjectType[] }, AWSError> | CallbackType<DynamoDB.BatchWriteItemInput, AWSError>
  	...
  	const prepareResponse = async (response:DynamoDB.BatchWriteItemOutput):Promise<{ unprocessedItems:ObjectType[] }> => {
  		const unprocessedArray = response.UnprocessedItems && response.UnprocessedItems[this.name] ?
  			response.UnprocessedItems[this.name] :
  			[];
  		...
  		return documents.reduce((result:{ unprocessedItems:ObjectType[] }, document) => {
  			const item = tmpResultUnprocessed.find((item) => Object.keys(document)
  				.every((keyProperty) => item[keyProperty] === document[keyProperty]));
  			if (item) {
  	...
  	const paramsPromise:Promise<DynamoDB.BatchWriteItemInput> = (
  		async ():Promise<DynamoDB.BatchWriteItemInput> => (
  			{
  				"RequestItems": {
  					[this.name]: await Promise.all(documents.map(async (document) => (
  						{
  							"PutRequest": {
  								"Item": await new this.Document(document as any).toDynamo({
  									"defaults": true,
  									"validate": true,
  									"required": true,
  									"enum": true,
  	...
  	if (callback) {
  		...
  		return (
  			async ():Promise<{ unprocessedItems:ObjectType[] }> => {
  				const response = await promise;
  ...
  batchDelete (
    keys:InputKey[],
    settings:ModelBatchDeleteSettings & { "return":"request" },
    callback:CallbackType<DynamoDB.BatchWriteItemInput, AWSError>
  ):void;
  ...
  batchDelete (
    keys:InputKey[],
    settings:ModelBatchDeleteSettings & { "return":"response" }
  ):Promise<{ unprocessedItems:ObjectType[] }>;
  batchDelete (
    keys:InputKey[],
    settings:ModelBatchDeleteSettings & { "return":"response" },
  ...
  batchDelete (
  	keys:InputKey[],
  	settings?:ModelBatchDeleteSettings | CallbackType<{ unprocessedItems:ObjectType[] }, AWSError> | CallbackType<DynamoDB.BatchWriteItemInput, AWSError>,
  	callback?:CallbackType<{ unprocessedItems:ObjectType[] }, AWSError> | CallbackType<DynamoDB.BatchWriteItemInput, AWSError>
  ):void | DynamoDB.BatchWriteItemInput | Promise<{ unprocessedItems:ObjectType[] }> {
  	if (typeof settings === "function") {
  		callback = settings;
  		settings = {"return": "response"};
  	}
  	...
  	const prepareResponse = async (response:DynamoDB.BatchWriteItemOutput):Promise<{ unprocessedItems:ObjectType[] }> => {
  		...
  		return keyObjects.reduce((result, key) => {
  			const item = tmpResultUnprocessed.find((item) => Object.keys(key)
  				.every((keyProperty) => item[keyProperty] === key[keyProperty]));
  			if (item) {
  				result.unprocessedItems.push(item);
  			}
  			return result;
  ...
  update (
    ...
    callback:CallbackType<T, AWSError>
  ):void;
  ...
  update (
  	keyObj:ObjectType,
  	updateObj?:Partial<T> | CallbackType<T, AWSError> | CallbackType<DynamoDB.UpdateItemInput, AWSError>,
  	settings?:ModelUpdateSettings | CallbackType<T, AWSError> | CallbackType<DynamoDB.UpdateItemInput, AWSError>,
  	callback?:CallbackType<T, AWSError> | CallbackType<DynamoDB.UpdateItemInput, AWSError>
  ):void | Promise<T> | Promise<DynamoDB.UpdateItemInput> {
  	...
  	const getUpdateExpressionObject:() => Promise<any> = async () => {
  		...
  		const returnObject = await Object.keys(updateObj).reduce(
  			async (accumulatorPromise, key) => {
  				const accumulator = await accumulatorPromise;
  				let value = updateObj[key];

  				if (!(
  					typeof value === "object" && updateTypes.map((a) => a.name).includes(key)
  				)) {
  					value = {[key]: value};
  					key = "$SET";
  				}
  				...
  				for (let i = 0; i < valueKeys.length; i++) {
  					...
  					if (!updateType.attributeOnly && subValue !== dynamooseUndefined) {
  						subValue =
                (
                	await this.Document.objectFromSchema(
                		{
                			[subKey]: dynamoType === "L" && !Array.isArray(subValue) ?
                				[subValue] :
                				subValue
                		},
                		this,
                    {
                    	"type": "toDynamo",
                    	"customTypesDynamo": true,
                    	"saveUnknown": true, ...updateType.objectFromSchemaSettings
                    } as any
                	)
                )[subKey];
  					}
  					...
  					const operator = updateType.operator ||
              (
              	updateType.attributeOnly ? "" : " "
              );
  			...
  			Promise.resolve((
  				async ():Promise<{ ExpressionAttributeNames:ObjectType; ExpressionAttributeValues:ObjectType; UpdateExpression:ObjectType }> => {
  					const obj = {
  					...
  					const documentFunctionSettings:DocumentObjectFromSchemaSettings = {
  						"updateTimestamps": {"updatedAt": true},
  						"customTypesDynamo": true,
  						"type": "toDynamo"
  					};
  					const defaultObjectFromSchema = await this.Document.objectFromSchema(await this.Document.prepareForObjectFromSchema(
  						{},
  						this,
  						documentFunctionSettings
  					), this, documentFunctionSettings);
  		...
  		schema.attributes().map((attribute) => {
  			...
  			const {invalidAttributes} = details.type.typeSettings.attributes.reduce((result, attribute) => {
  				const expressionAttributeNameEntry = Object.entries(returnObject.ExpressionAttributeNames)
  					.find((entry) => entry[1] === attribute);
  				const doesExist = Boolean(expressionAttributeNameEntry);
  				const isValid = doesExist &&
            [...returnObject.UpdateExpression.SET, ...returnObject.UpdateExpression.REMOVE].join(", ")
            	.includes(expressionAttributeNameEntry[0]);
  			...
  			if (invalidAttributes.length > 0) {
  				...
  				const nextIndex = Math.max(...Object.keys(returnObject.ExpressionAttributeNames)
  					.map((key) => parseInt(key.replace("#a", "")))) + 1;
  				returnObject.ExpressionAttributeNames[`#a${nextIndex}`] = details.attribute;
  				returnObject.ExpressionAttributeValues[`:v${nextIndex}`] =
            details.type.typeSettings.attributes.map((attribute) => {
            	const [expressionAttributeNameKey] = Object.entries(returnObject.ExpressionAttributeNames)
            		.find((entry) => entry[1] === attribute);
            	return returnObject.ExpressionAttributeValues[expressionAttributeNameKey.replace("#a", ":v")];
            })
            	.filter((value) => typeof value !== "undefined" && value !== null)
            	.join(details.type.typeSettings.seperator);
  	...
  	const documentify = (document):Promise<any> => new this.Document(
  		document,
  		{"type": "fromDynamo"}
  	).conformToSchema({"customTypesDynamo": true, "checkExpiredItem": true, "type": "fromDynamo"});
  	const localSettings:ModelUpdateSettings = settings;
  	const updateItemParamsPromise:Promise<DynamoDB.UpdateItemInput> = this.pendingTaskPromise()
  		.then(async () => (
  			{
  				"Key": this.Document.objectToDynamo(keyObj),
  				"ReturnValues": "ALL_NEW",
  				...utils.merge_objects.main({"combineMethod": "object_combine"})(
  					localSettings.condition ? localSettings.condition.requestObject({
  						"index": {
  							"start": index, "set": (i):void => {
  								index = i;
  							}
  						}, "conditionString": "ConditionExpression", "conditionStringType": "string"
  					}) : {},
  	...
  	if (callback) {
  		promise.then((response) => response.Attributes ? documentify(response.Attributes) : undefined)
  ...
  create (
  	document:Partial<T>,
  	settings?:DocumentSaveSettings | CallbackType<T, AWSError> | CallbackType<DynamoDB.PutItemInput, AWSError>,
  	callback?:CallbackType<T, AWSError> | CallbackType<DynamoDB.PutItemInput, AWSError>
  ):void | Promise<T> | Promise<DynamoDB.PutItemInput> {
  ...
  delete (
  	key:InputKey,
  	settings?:ModelDeleteSettings | CallbackType<void, AWSError> | CallbackType<DynamoDB.DeleteItemInput, AWSError>,
  	callback?:CallbackType<void, AWSError> | CallbackType<DynamoDB.DeleteItemInput, AWSError>
  ):void | DynamoDB.DeleteItemInput | Promise<void> {
  	if (typeof settings === "function") {
  		callback = settings;
  ...
  get (
  	key:InputKey,
  	settings?:ModelGetSettings | CallbackType<T, AWSError> | CallbackType<DynamoDB.GetItemInput, AWSError>,
  	callback?:CallbackType<T, AWSError> | CallbackType<DynamoDB.GetItemInput, AWSError>
  ):void | DynamoDB.GetItemInput | Promise<T> {
  	if (typeof settings === "function") {
  		callback = settings;
  		settings = {"return": "document"};
  	}
  	...
  	const conformToSchemaSettings:DocumentObjectFromSchemaSettings = {
  		"customTypesDynamo": true,
  		"checkExpiredItem": true,
  		"saveUnknown": true,
  		"modifiers": ["get"],
  	...
  	if (settings.attributes) {
  		...
  		getItemParams.ExpressionAttributeNames =
        settings.attributes.reduce((accumulator, currentValue, index) => (
        	accumulator[`#a${index}`] = currentValue, accumulator
        ), {});
  	}
  	if (settings.return === "request") {
  		if (callback) {
  	...
  	if (callback) {
  		const localCallback:CallbackType<DocumentCarrier, AWSError> = callback as CallbackType<DocumentCarrier, AWSError>;
  		promise.then((response) => response.Item ? documentify(response.Item) : undefined)
  			.then((response) => localCallback(null, response))
  			.catch((error) => callback(error));
  	} else {
  		return (
...
const customMethodFunctions = (type:"model" | "document"):{
  set:(
	...
	const entryPoint = (self:Model<DocumentCarrier>):DocumentCarrier | typeof DocumentCarrier => type === "document" ?
		self.Document.prototype :
		self.Document;
	return {
		"set": function (name:string, fn):void {
			const self:Model<DocumentCarrier> = this as any;
			if (!entryPoint(self)[name] ||
        entryPoint(self)[name][Internal.General.internalProperties] &&
        entryPoint(self)[name][Internal.General.internalProperties].type ===
        "customMethod") {
